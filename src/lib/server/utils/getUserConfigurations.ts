...
export async function getUserConfigurations(userId: number) {
	...
	const activeConnectionId = userSettings?.activeConnectionId ?? systemSettings?.defaultConnectionId
	...
	const activeSamplingConfigId = userSettings?.activeSamplingConfigId ?? systemSettings?.defaultSamplingConfigId
	...
	const activeContextConfigId = userSettings?.activeContextConfigId ?? systemSettings?.defaultContextConfigId
	...
	const activePromptConfigId = userSettings?.activePromptConfigId ?? systemSettings?.defaultPromptConfigId
	...
	if (!activeConnection || !activeSamplingConfig || !activeContextConfig || !activePromptConfig) {
		throw new Error(`Missing required configuration for user ${userId}: ${!activeConnection ? 'connection ' : ''}${!activeSamplingConfig ? 'sampling ' : ''}${!activeContextConfig ? 'context ' : ''}${!activePromptConfig ? 'prompt' : ''}`)
	}
